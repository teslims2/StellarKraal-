...
async function flushHealthFetch() {
  await act(async () => {
...
describe('useHealthFactor', () => {
  ...
  it('fetches immediately when a loan ID is provided', async () => {
    mockHealth(12000);
    const { result } = renderHook(() => useHealthFactor('42'));

    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/api/health/42'));
    await flushHealthFetch();
    expect(result.current.healthFactor).toBe(12000);
  });

  it('polls for health data every 30 seconds', async () => {
    mockHealth(12000);
    const { result } = renderHook(() => useHealthFactor('42'));
  ...
  it('clears the polling interval on unmount', async () => {
    mockHealth(12000);
    const { result, unmount } = renderHook(() => useHealthFactor('42'));
    await flushHealthFetch();
    expect(result.current.healthFactor).toBe(12000);
  ...
  it('loading state: is true while fetch is pending', async () => {
    ...
    expect(result.current.loading).toBe(true);
    act(() => {
      resolve({ ok: true, status: 200, json: async () => ({ health_factor: 15000 }) });
    });
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.healthFactor).toBe(15000);
  });
