  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      ...
      <p className="text-brown/60 mb-6 text-sm">
        ...
        <a href="https://github.com/teslims2/StellarKraal-/issues" target="_blank" rel="noopener noreferrer" className="underline hover:text-brown">
          Open a GitHub issue
        </a>.
      </p>
      <input
      ...
      <div className="flex flex-wrap gap-2 mb-8" role="tablist">
        <button role="tab" aria-selected={activeCategory === null} onClick={() => setActiveCategory(null)}
          className={`px-3 py-1.5 rounded-full text-sm font-medium transition ${activeCategory === null ? "bg-brown text-cream" : "bg-brown/10 text-brown hover:bg-brown/20"}`}>
          All
        </button>
        {FAQ_CATEGORIES.map((cat) => (
          <button key={cat.id} role="tab" aria-selected={activeCategory === cat.id}
            onClick={() => setActiveCategory(activeCategory === cat.id ? null : cat.id)}
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition ${activeCategory === cat.id ? "bg-brown text-cream" : "bg-brown/10 text-brown hover:bg-brown/20"}`}>
            {cat.label}
          </button>
        ))}
      </div>
      {filtered.length === 0 ? (
        <p className="text-brown/60 text-center py-12">No results for &ldquo;{query}&rdquo;.</p>
      ) : (
        filtered.map((cat) => (
          <section key={cat.id} className="mb-10">
            <h2 className="text-lg font-semibold text-brown mb-4 border-b border-brown/10 pb-2">{cat.label}</h2>
